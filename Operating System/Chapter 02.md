...
- update가 용이하다!
단점:
- 동시에 존재할때 호환성 문제! - 버전 관리 어려움
- 의존성 문제 (DLL 지옥)
- 보안 문제
- 성능 오버헤드 - 파일을 로드하고 링크하는 과정에서 성능 오버헤드 발생
...
| **장점**        | 높은 성능                    | 높은 안정성과 보안성, 유연성, 커널의 사이즈가 작아진다. 확장성이 높다 | 구조의 명확성, 유지보수와 개발 용이성 | 시스템 재부팅 없이 기능 추가 및 제거 가능      |
...
| 예시            | Linux, Unix              | Darwin                                   | OSI 7 Layer           | Linux                         |
...
### Core Dump
1. **정의**: Core dump는 프로그램이 비정상적으로 종료될 때 발생하는 이벤트로, 프로그램의 실행 중에 메모리에 있던 내용을 파일로 저장하는 것입니다. 이 파일을 분석함으로써 프로그램이 충돌한 원인을 파악할 수 있습니다.
2. **목적**: 프로그램의 충돌 시점에서의 메모리 상태, 즉 변수의 값, 실행 중이던 함수의 호출 스택 등을 포함하므로, 디버깅 시 문제의 원인을 파악하는 데 매우 유용합니다.
3. **사용법**: 대부분의 유닉스 및 리눅스 기반 시스템에서는 core dump 파일이 자동으로 생성되며, 이 파일은 'core' 또는 'core.프로세스ID'와 같은 이름을 가집니다. 개발자는 gdb와 같은 디버깅 도구를 사용하여 이 파일을 분석할 수 있습니다.
4. **제한사항**: Core dump는 일반적으로 특정 프로세스의 충돌 정보만을 포함합니다. 전체 시스템이 아니라, 특정 프로그램 또는 프로세스의 문제를 진단하는 데 중점을 둡니다.
    
### Crash Dump
1. **정의**: Crash dump는 시스템 전체가 비정상적으로 종료됐을 때, 즉 시스템 크래시가 발생했을 때 그 시점의 시스템 메모리 내용을 저장하는 것입니다. 보통 운영체제의 커널 오류로 인해 발생합니다.
2. **목적**: 시스템의 크래시 원인을 분석하기 위해 사용됩니다. Crash dump 파일에는 시스템의 메모리, 실행 중이던 프로세스, 시스템 상태, 커널 스택 등의 정보가 포함됩니다.
3. **사용법**: 윈도우에서는 "메모리 덤프 파일"이라고도 하며, 'Minidump' 또는 'Full dump' 등의 형식으로 저장됩니다. 윈도우에서는 BlueScreen이 발생했을 때 자동으로 생성되며, Dump 파일은 "C:\Windows\Minidump" 또는 "C:\Windows" 디렉토리에 저장됩니다. 분석을 위해 WinDbg와 같은 도구가 사용됩니다.
4. **제한사항**: Crash dump는 전체 시스템의 메모리를 저장하기 때문에 파일 크기가 매우 클 수 있습니다. 또한, 분석을 위해서는 상대적으로 고급의 디버깅 지식이 필요합니다.
    

### 결론

Core dump와 Crash dump 모두 프로그램이나 시스템의 비정상 종료 원인을 분석하기 위한 중요한 도구입니다. Core dump는 특정 프로세스의 문제를 진단하는 데 사용되며, Crash dump는 시스템 전체의 크래시를 분석하는 데 사용됩니다. 각각의 덤프는 프로그램 또는 시스템의 문제를 해결하는 데 있어 귀중한 정보를 제공합니다.
...
## 1. Windows
- Monolithic - 가장큼
- MicroKernel의 행동을 취한다
## 2. MacOS/Ios
- Layered 구조
- User Experience Layer와 Applicaiton Framework Layer / Core
- CoCoa Framework , fetch Framework
...
- 오픈소스
- 자바 기반이다! 
- 스탠다드 자바가 아니라 구글이 따로 디자인한 따른 자바 뭐가 있다

## 어셈블리어 -> C 로 바뀐이유
- 디버깅쉽고 이해하기 좋다
- 코드 작성 자체가 이해하기 좋지만
- 대신 속도가 느리고 계산 공간을 많이 차지한다!